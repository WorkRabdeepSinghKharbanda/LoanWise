...
- **Ads (Google AdSense)** are off. [src/config/ads.ts](./src/config/ads.ts) holds the `enabled` flag, reserved slot heights, and each slot's real AdSense `adSlotId` (`homeTop`, `resultsBottom`, `footer`, `articleMid` — the last used on guide/blog pages); [src/config/adsense.ts](./src/config/adsense.ts) holds the publisher id (`ADSENSE_PUBLISHER_ID`) and `isAdsConfigured()`. The AdSense loader script itself is a static `<script>` tag in `index.html` — it loads unconditionally on every page (per AdSense's own setup instructions), not gated on cookie consent. `<AdSlot name="…"/>` renders nothing while `ADS.enabled` is false, reserving space either way so switching ads on causes no layout shift; once enabled + configured + the slot has a real `adSlotId`, it renders a real `<ins class="adsbygoogle">` unit and pushes it — otherwise the placeholder box. [CookieConsentBanner](./src/components/CookieConsentBanner.tsx) (tracked via [src/utils/consent.ts](./src/utils/consent.ts)) is disclosure-only — it records the visitor's choice so it doesn't reappear, but doesn't gate the script or any `AdSlot`. To go live: replace `ADSENSE_PUBLISHER_ID`, fill in each slot's `adSlotId`, replace the placeholder ids in [public/ads.txt](./public/ads.txt), then set `ADS.enabled = true`. [Privacy policy](./src/pages/PrivacyPage.tsx) (`/privacy`, linked from the footer) covers what's stored and how ads use cookies.
